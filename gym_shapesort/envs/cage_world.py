...
def init():
	...
	floor 	  = p.loadURDF(plane_desc, [0, 0, -0.6])
	wall_left = p.loadURDF(wall_desc,  [0, wall_distance/2, -0.1])
	wall_right = p.loadURDF(wall_desc, [0, -wall_distance/2, -0.1])

	pi_2_rot = p.getQuaternionFromEuler([0, 0, math.pi/2])
	wall_front = p.loadURDF(wall_desc, [wall_distance/2, 0, -0.1], pi_2_rot)
