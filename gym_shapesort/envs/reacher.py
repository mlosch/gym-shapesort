...
class Reacher(gym.Env):
	...
	def __init__(self, frame_skip=2, obs_type='rgb', render_width=112, render_height=112, pybullet_mode='SHARED_MEMORY'):
	...
	def _get_obs(self):
		...
		obs = np.empty((self.render_height, self.render_width, channels*len(self.camera_pos)))
		...
		for i, camera_pos in enumerate(self.camera_pos):
			...
			if self.obs_type == 'rgbd':
				...
				rgba = rgba[:,:,:3]  # slice off alpha, always 1.0
	...
	def _reset_target(self):
		...
		randpos = np.array([randf(-0.9, 0.9), randf(-0.9, 0.9), randf(-0.5, 0.5)])
	...
	def _render(self, mode='human', close=False):
		...
		# self.window.clear()
		# pyglet.sprite.Sprite(Image.fromarray(self._get_obs(), 'RGB')).draw()

		# from scipy.misc import imshow
		# imshow(self._get_obs())
		if self.obs_type == 'q':
			obs = self._get_obs()
	...
	def observation_space(self):
		if self.obs_type == 'q':
			...
			if self.obs_type == 'rgbd':
				...
				channels = 6
			high = np.ones((self.render_height, self.render_width, channels))
			low  = -np.ones((self.render_height, self.render_width, channels))
			return spaces.Box(low, high)
